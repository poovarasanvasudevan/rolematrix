...
class CtsActiveDirectoryUserAddGroupsV2
  ...
  def initialize(input)
    ...
    if @parameters['groups'].nil? || @parameters['groups'].empty?
  ...
  def execute()
    ...
    unless @group_available
      @status = "Failed"
      @results = "Group name is empty"
      results = <<-RESULTS
           <results>
    ...
    if @parameters['search_value'].nil? || @parameters['search_value'].empty?
      @status = "Failed"
      @results = "Search Value is empty"
      results = <<-RESULTS
		   <results>
		   <result name="Status">#{@status}</result>
		   <result name="Result">#{@results}</result>
    ...
    if @ldap.bind
      ...
      if user_entries.length < 1
        ...
        @status = "Failed"
        @results = "User not found when searching by #{@search_by} for: #{@parameters['search_value']}"
        ...
        @status = "Failed"
        @results = "Search matched more than one entry when searching by #{@search_by} for: #{@parameters['search_value']}"
      end
      if @status == "Successful"
        ...
        if missing_group_names.length > 0
          ...
          @status = "Failed"
          @results = "The following groups did not exist on the configured Active Directory server: #{missing_group_names.join(', ')}"
          ...
          @results = "The following groups were added to #{user_dn}:\n"
          groups.each do |name, group|
            @results = @results + "  #{name}: #{group.dn}\n"
          end
      ...
      @status = "Failed"
      @results = "Service account authentication failed for #{@info_values['host']}: #{@ldap.get_operation_result.message}"
    end
